// this file uses JavaDoc style comment blocks for automatic extraction 
...
//!A vector class.
/*!
The vector class incapsulates a classic C++ vector of three real values and 
treats them as coordinates of a 3 dimensional mathematical vector.

The most common operations between mathematical vectors (and some which involves 
scalars too) are defined.
*/
class vector
{
private:
...
The most common operators between matrices (and some which involves scalars and 
vectors too) are defined.
*/
class tmatrix
{
private:
...
public:
	//! Expresses how to store a tmatrix in a single 16 elements vector, by column or by row
	enum ordermode
	{
...
The quaternion uses internally the (s,<B>v</B>) representation, where s is a 
scalar and <B>v</B> a vector.
