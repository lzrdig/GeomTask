...
void CClassArray<TYPE, ARG_TYPE>::SetSize(int nNewSize, int nGrowBy)
{
	...
	else
	{
		...
		int nGrowByLocal = m_nGrowBy;
		if (nGrowByLocal == 0)
		{
			// heuristically determine growth when nGrowByLocal == 0
			//  (this avoids heap fragmentation in many situations)
			nGrowByLocal = m_nSize / 8;
			nGrowByLocal = (nGrowByLocal < 4) ? 4 : ((nGrowByLocal > 1024) ? 1024 : nGrowByLocal);
		}
		int nNewMax;
		if (nNewSize < m_nMaxSize + nGrowByLocal)
			nNewMax = m_nMaxSize + nGrowByLocal;  // granularity
		else
